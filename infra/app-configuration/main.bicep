...
@description('Optional. Feature flags to create.')
...
module configurationStore_featureFlags '.bicep/feature_flags.bicep' = [for (featureFlag, index) in featureFlags: {
  name: '${deployment().name}-featureFlags-${index}'
  params: {
    appConfigurationName: configStore.name
